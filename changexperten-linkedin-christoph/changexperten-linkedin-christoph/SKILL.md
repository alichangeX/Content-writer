...
**Mapping für die Airtable-Pipeline:** Die Tabelle „Content-Research" kennt noch die alten Kategorie-Labels. Übersetzung bei Bedarf: Leadership Thought/Personal Hot Take → Meinungs-Post oder Zitat-Konfrontation; Actionable Guide/Trends & Analytics → Kontrast-Liste oder Zitat-Konfrontation mit Studien-Rückgrat; Client Success Story → Fall-Story; Personal Milestone/Company Wins → Persönlicher Meilenstein; Offer Highlight → entfällt als eigenes Format, Angebote laufen als Nebensatz oder PS mit (siehe Soft-Sell-Regel). Die Labels in Airtable müssen nicht umgebaut werden, sie sind nur noch Herkunftsvermerk.
...
2 Posts pro Woche, Montag und Mittwoch, Veröffentlichung 07:00 über das Make-Szenario „Content-Publishing" (ID 9521520). Fällt ein Slot aus, wird er nicht nachgeholt. In der Golden Hour (07:00 bis 08:30) muss Christoph für Kommentare erreichbar sein, dazu 20 bis 30 Minuten Kommentieren in fremden Feeds; ist das absehbar nicht möglich, lieber den Slot ausfallen lassen als unbegleitet zu posten. Die Benchmark stützt diese Frequenz: Das Profil mit der stabilsten Grundperformance der Analyse postet ca. 2x pro Woche – Haltung und Persönlichkeit schlagen Frequenz. Eine dritte wöchentliche Ausbaustufe ist möglich, wenn die Nachbereitung real stattfindet; sie braucht keine eigene Quotenmechanik mehr.
...
Sonst:
1. **Vorfall-Speicher zuerst (Vorfall-vor-Thema-Prinzip):** Tabelle „Vorfälle" in der Base „ChangeXperten" (`appb7eOfe2Au3Lp40`) prüfen. Dort sammelt Christoph Rohmomente aus Kundenterminen, Trainings und EO-Abenden (wörtliches Zitat, krumme Zahl, absurdes Detail, Datum, ggf. Artefakt-Hinweis). Auswahllogik: vorhandener Vorfall → passende Studie aus der Pipeline dazu, nicht umgekehrt. Existiert die Tabelle nicht oder ist sie leer, das offen sagen und die Vorfall-Fragen aus Schritt 3 stellen.
...
- Base „ChangeXperten" (`appb7eOfe2Au3Lp40`), Tabelle „LinkedIn-Bilder" (`tblt8MncEzwvhIQ8C`), Zielfeld „Bildvorschläge LinkedIn" (`fldBQwjgyFZw3SNwr`) in Tabelle „Content-Research" (`tblrCjOdKhI4YCPPR`)
...
- Nutzungszähler und „Zuletzt genutzt" fasst ausschließlich das Make-Szenario „Content-Publishing" (ID 9521520) an.
